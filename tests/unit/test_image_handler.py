...
def test_enqueue_image_to_image_with_image_object(image_handler, mock_image, mock_info):
    with patch.object(image_handler.queue, "put") as mock_put:
        image_handler._enqueue_image_to_image(info=mock_info, image=mock_image)
...
def test_enqueue_image_to_image_with_image_url(image_handler, mock_image, mock_info):
    with (
        ...
        url = "http://fakeurl.com/image.jpg"
        image_handler._enqueue_image_to_image(info=mock_info, image=url)
...
def test_enqueue_image_to_image_with_optional_params(image_handler, mock_info, mock_image):
    with patch.object(image_handler.queue, "put") as mock_put:
        ...
        image_handler._enqueue_image_to_image(
            info=mock_info,
            image=valid_image,
            prompt="test prompt",
            num_inference_steps=50,
...
def test_enqueue_image_to_image_task_structure(image_handler, mock_info, mock_image):
    with patch.object(image_handler.queue, "put") as mock_put:
        valid_image = mock_image
        image_handler._enqueue_image_to_image(info=mock_info, image=valid_image)
        assert mock_put.called
        task = mock_put.call_args[0][0]
        assert "type" in task
